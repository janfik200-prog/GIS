...
# вклады частей модели
W_GEO = 0.78
W_CLUSTER = 0.14
W_ML = 0.08 if USE_SUPERVISED else 0.0
...
OUT_DIR = BASE_DIR / "same_methods_fixed_v2_result"
OUT_DIR.mkdir(parents=True, exist_ok=True)
SAFE_ALIAS_DIR = OUT_DIR / "_safe_shp_aliases"
SAFE_ALIAS_DIR.mkdir(parents=True, exist_ok=True)

OUT_GPKG = OUT_DIR / "gold_forecast_same_methods_fixed_v2.gpkg"
OUT_PNG = OUT_DIR / "gold_forecast_same_methods_fixed_v2.png"
OUT_PROX = OUT_DIR / "prox_magm_same_methods_fixed_v2.png"
OUT_COMPARE = OUT_DIR / "compare_same_methods_fixed_v2.png"
OUT_CSV = OUT_DIR / "grid_attributes_same_methods_fixed_v2.csv"
OUT_JSON = OUT_DIR / "metrics_same_methods_fixed_v2.json"
...
def smooth_on_regular_grid(grid, value_col, shape, passes=1):
    ...
    kernel = np.array([[1.0, 1.2, 1.0], [1.2, 3.2, 1.2], [1.0, 1.2, 1.0]], dtype=float)
    smoothed = arr.copy()
...
def make_display_classes(grid, source_col="prognoz"):
    # Для сравнения с эталоном делаем отдельную display-нормализацию,
    # а raw значения оставляем как есть в таблицах.
    disp = robust_normalize_01(1.0 - grid[source_col].to_numpy(), 0.03, 0.97)
    grid["display_score"] = disp
    bins = np.linspace(0, 1, N_DISPLAY_CLASSES + 1)
    cls = np.digitize(disp, bins[1:-1], right=False)
    grid["display_class"] = cls
    # Жёлтые лучшие зоны: верхние классы по display_score
    grid["display_top"] = (grid["display_class"] >= (N_DISPLAY_CLASSES - TOP_CLASS_COUNT)).astype(int)
    return grid


def get_display_cmap():
    base = plt.cm.get_cmap("bwr", N_DISPLAY_CLASSES)
    colors = base(np.linspace(0, 1, N_DISPLAY_CLASSES))
    # верхние классы красим в желтые/золотистые оттенки
    yellow_palette = [
        np.array([1.0, 0.95, 0.35, 1.0]),
        np.array([1.0, 0.88, 0.15, 1.0]),
        np.array([0.95, 0.78, 0.05, 1.0]),
    ]
    for i in range(TOP_CLASS_COUNT):
        colors[-TOP_CLASS_COUNT + i] = yellow_palette[i]
...
def plot_final(grid, mask, points, out_png):
    ...
    grid.plot(column="display_class", ax=ax, cmap=cmap, linewidth=0, legend=True)
    mask.boundary.plot(ax=ax, color="black", linewidth=0.5)
    if points is not None and len(points) > 0:
        points.plot(ax=ax, color="yellow", markersize=10, edgecolor="black", linewidth=0.3)
    set_mask_extent(ax, mask)
...
def plot_compare(grid, mask, points, out_png):
    ...
    cmap = get_display_cmap()
    grid.plot(column="display_class", ax=axes[1], cmap=cmap, linewidth=0)
    mask.boundary.plot(ax=axes[1], color="black", linewidth=0.5)
    if points is not None and len(points) > 0:
        points.plot(ax=axes[1], color="yellow", markersize=9, edgecolor="black", linewidth=0.3)
...
# GEO SCORE
# =========================================================
# Более контрастный, но без смены твоей общей логики.
grid["geo_score_raw"] = (
    0.21 * grid["prox_tect1"] +
    0.21 * grid["prox_tect2"] +
    0.14 * grid["prox_paleo"] +
    0.12 * grid["prox_struct"] +
    0.10 * grid["prox_facies"] +
    0.10 * grid["prox_magm"] +
    0.05 * grid["tect_intersection"] +
    0.04 * grid["tect_magm_intersection"] +
    0.03 * grid["tect_struct_intersection"]
)
...
if use_supervised:
    ...
    cluster_stats["cluster_score"] = robust_normalize_01(
        0.60 * cluster_stats["cluster_geo_mean"] +
    ...
    cluster_stats["cluster_score"] = robust_normalize_01(
        0.80 * cluster_stats["cluster_geo_mean"] + 0.20 * cluster_stats["cluster_ml_mean"],
        0.02, 0.98,
    )
...
# =========================================================
if use_supervised:
    grid["prospectivity_raw"] = 0.62 * grid["geo_score_sm"] + 0.15 * grid["cluster_score"] + 0.23 * grid["ml_score"]
else:
    grid["prospectivity_raw"] = W_GEO * grid["geo_score_sm"] + W_CLUSTER * grid["cluster_score"]

grid["local_bonus"] = robust_normalize_01(
    0.42 * grid["tect_intersection"] +
    0.33 * grid["tect_magm_intersection"] +
...
grid = make_display_classes(grid, source_col="prognoz")
...
print("Диагностика:")
print(grid[["prospectivity", "prognoz", "display_score"]].describe())
