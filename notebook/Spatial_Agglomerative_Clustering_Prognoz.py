...
OUT_DIR = BASE_DIR / "spatial_agglomerative_result"
...
def smooth_on_regular_grid(grid, values, shape, passes=1):
    from scipy.signal import convolve2d

    arr = np.full(shape, np.nan, dtype=float)
    arr[grid["row"].to_numpy(), grid["col"].to_numpy()] = np.asarray(values, dtype=float)
...
def local_max_mask(grid, value_col, shape, size=3):
...
def connected_component_filter(grid, mask_col, shape, min_cells=2):
    from scipy.ndimage import label

    arr = np.zeros(shape, dtype=bool)
    arr[grid["row"].to_numpy(), grid["col"].to_numpy()] = grid[mask_col].to_numpy().astype(bool)
    ...
    keep[0] = False
    return keep[labels][grid["row"].to_numpy(), grid["col"].to_numpy()]
...
def mark_top_zones(grid, shape):
    ...
    q_core = float(grid["prospectivity"].quantile(0.97))
    q_signal = float(grid["signal_local"].quantile(0.90))
    local_peak = local_max_mask(grid, "prospectivity", shape, size=3)

    grid["top_zone"] = (
        (grid["prospectivity"] >= q_support) &
        local_peak &
        (grid["prospectivity"] >= q_core) &
        (grid["signal_local"] >= q_signal)
    ).astype(int)

    grid["top_zone"] = connected_component_filter(grid, "top_zone", shape, min_cells=1).astype(int)
    return grid
...
def collect_points(mask_crs, aliases):
    ...
    for name, shp_path in aliases.items():
        ...
        info.append({"layer": name, "n": len(gdf), "geom_types": sorted(list(types))})
...
feature_preview = [
    ...
    "tect_only_penalty", "signal_base", "signal_local"
]
grid[feature_preview].describe().T
...
cluster_features = [
    ...
    "prox_tect1", "prox_tect2", "tect_intersection", "tect_magm_intersection",
    "tect_struct_intersection", "paleo_struct_intersection", "coincidence_score",
    "tect_only_penalty", "signal_base", "signal_local", "tect_local", "magm_struct_local"
]
...
print("Количество кластеров:", grid["cluster"].nunique())
print("Размеры кластеров:", np.bincount(labels))



cluster_summary = (
    grid.groupby("cluster")[cluster_features + ["signal_base", "signal_local"]]
    .mean()
...
cluster_summary["cluster_score_raw"] = (
    0.12 * cluster_summary["prox_tect1"] +
    0.12 * cluster_summary["prox_tect2"] +
    0.11 * cluster_summary["prox_struct"] +
    0.10 * cluster_summary["prox_paleo"] +
    0.10 * cluster_summary["prox_magm"] +
    0.08 * cluster_summary["prox_facies"] +
    0.10 * cluster_summary["tect_intersection"] +
    0.10 * cluster_summary["tect_magm_intersection"] +
    0.07 * cluster_summary["tect_struct_intersection"] +
    0.05 * cluster_summary["paleo_struct_intersection"] +
    0.10 * cluster_summary["coincidence_score"] -
    0.05 * cluster_summary["tect_only_penalty"]
)
...
cluster_summary["cluster_score"] = robust_normalize_01(
    0.85 * cluster_summary["cluster_score_raw"] + 0.15 * cluster_summary["size_bonus"],
    0.02, 0.98
)
...
grid["cluster_percentile"] = grid.groupby("cluster")["signal_base"].rank(method="average", pct=True)

grid["prospectivity_raw"] = (
    0.70 * grid["cluster_base"] +
    0.15 * grid["cluster_percentile"] +
    0.15 * grid["signal_local"]
)
...
cluster_summary[["cluster", "cluster_size", "cluster_score", "cluster_score_raw"]]
...
OUT_GPKG = OUT_DIR / "spatial_agglomerative_forecast.gpkg"
OUT_PNG = OUT_DIR / "spatial_agglomerative_forecast.png"
OUT_CSV = OUT_DIR / "cluster_summary.csv"
